...
export const googlePhotos: SystemDesign = {
  slug: 'google-photos',
  name: 'Google Photos',
  ...
  overview: `Google Photos is a photo and video storage service used by 1B+ people, storing over 4 trillion photos. Its architecture uniquely combines massive media storage with deep AI/ML capabilities — automatic organization by people, places, and things; natural language search ("dog on beach"); and AI-generated creations (collages, animations, memories). The system must handle high-throughput media uploads, on-the-fly transcoding for diverse device displays, efficient deduplication, and ML inference at scale for every uploaded photo. Google Photos is one of the largest deployments of computer vision in the world.`,
  scale: {
    'Users': '1B+',
    'Photos stored': '4T+',
  ...
  highLevelDiagram: `graph TB
    ...
    CDN --> BLOB`,
  components: [
    { name: 'Upload Service', description: 'Handles photo/video upload with resumable uploads and adaptive quality. Deduplicates uploads using perceptual hashing (not just content hash) to detect near-identical photos. Queues uploaded media for processing.' },
    { name: 'Media Processing Pipeline', description: 'Transcodes uploaded media into multiple formats and resolutions — thumbnails (256px), previews (1024px), and full resolution. Videos are transcoded to H.264/VP9. Uses content-aware compression to reduce storage while maintaining visual quality.' },
    { name: 'ML Pipeline', description: 'Runs 10+ ML models on every uploaded photo: face detection/recognition, object/scene classification, OCR, geolocation estimation from visual features, aesthetic quality scoring, and explicit content detection. Processes 1.2B+ photos per day.' },
    { name: 'Face Recognition', description: 'Detects faces in photos and clusters them into identity groups using deep metric learning (face embeddings). Users can label clusters with names, enabling "photos of Alice" search. Uses ArcFace-style embeddings with a per-user face index.' },
    { name: 'Search Service', description: 'Supports natural language queries ("sunset at the beach", "birthday cake 2023") by mapping query terms to visual concepts, detected objects, OCR text, and metadata (date, location). Combines text matching with visual feature similarity search.' },
    { name: 'Memories Engine', description: 'Generates personalized "Memories" (e.g., "This day 3 years ago") by analyzing date patterns, location clustering, face groups, and aesthetic quality scores. Filters out low-quality, duplicate, or sensitive photos.' },
    { name: 'Sharing Service', description: 'Manages photo sharing, shared albums, and partner sharing (automatic sharing of photos containing specific people). Shared albums have their own permission model independent of Google Drive.' },
  ],
  dataModel: `erDiagram
    PHOTO {
        string photo_id PK
