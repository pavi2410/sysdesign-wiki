...
export const slack: SystemDesign = {
  slug: 'slack',
  name: 'Slack',
  ...
  highLevelDiagram: `graph TB
    ...
    NOTIF --> MQ`,
  components: [
    { name: 'Real-time Gateway', description: 'Maintains WebSocket connections to all online clients. Uses a channel-based pub/sub model — when a message is posted, the gateway fans it out to all connected members of that channel in real time.' },
    { name: 'Message Service', description: 'Handles message creation, editing, deletion, and threading. Messages are stored in MySQL (sharded via Vitess) and indexed asynchronously into Elasticsearch for search.' },
    { name: 'Channel Service', description: 'Manages channel metadata, membership, and permissions. Supports public channels, private channels, DMs, group DMs, and Slack Connect shared channels across organizations.' },
    { name: 'Search Service', description: 'Provides full-text search across messages, files, and channels using Elasticsearch. Supports filters (from:, in:, has:, before:, after:) and respects channel-level access controls.' },
    { name: 'File Service', description: 'Handles file uploads, virus scanning, thumbnail generation, and CDN distribution. Files are stored in S3-compatible object storage with metadata in MySQL.' },
    { name: 'Integration Service', description: 'Manages incoming/outgoing webhooks, slash commands, and the Slack App platform. Processes events from 2,600+ third-party integrations via a job queue.' },
    { name: 'Notification Service', description: 'Delivers push notifications, email digests, and badge counts. Respects per-channel notification preferences and Do Not Disturb schedules.' },
    { name: 'Presence Service', description: 'Tracks online/away/DND status for all users. Uses an in-memory cache with heartbeat-based expiry to keep presence state current.' },
  ],
  dataModel: `erDiagram
    WORKSPACE {
