...
export const e2eEncryption: FeatureGuide = {
  slug: 'e2e-encryption',
  ...
  problem: `Users expect their private messages, files, and data to be protected from everyone — including the service provider. End-to-end encryption (E2EE) ensures that only the communicating parties can read the content; the server handles encrypted blobs it cannot decrypt. Implementing E2EE requires solving key exchange (how do two parties agree on a shared secret over an insecure channel?), forward secrecy (compromising today's key shouldn't reveal past messages), multi-device synchronization (encryption keys across phone, laptop, tablet), and group encryption (efficiently encrypting for N participants). The server must be designed as an untrusted relay.`,
  approaches: [
    {
      name: 'Signal Protocol (Double Ratchet)',
      description: `The gold standard for messaging E2EE, used by WhatsApp, Signal, and Facebook Messenger. Combines the **X3DH** key agreement protocol with the **Double Ratchet Algorithm** to provide forward secrecy and break-in recovery. Each message uses a unique encryption key derived from a constantly evolving chain.`,
      pros: [
    ...
    },
    {
      ...
      description: `IETF standard (RFC 9420) designed for **efficient group encryption**. Where Signal Protocol requires O(N) operations for N-member groups, MLS uses a tree-based key structure achieving O(log N). Designed for large groups (thousands of members) and multi-device scenarios.`,
      pros: [
    ...
    },
    {
      ...
      description: `For encrypting stored data (files, backups, database fields), use **envelope encryption**: encrypt data with a data encryption key (DEK), then encrypt the DEK with a key encryption key (KEK) that the user controls. The server stores the encrypted DEK alongside the encrypted data but never has the plaintext KEK.`,
      pros: [
  ...
  architectureDiagram: `graph TB
    subgraph ClientA["Client A (Sender)"]
        ...
        A_KEYS[Key Store<br/>Local / Secure Enclave]
    end
    subgraph Server["Server (Untrusted Relay)"]
        KS[Key Distribution<br/>Service]
        MSG[Message Relay]
        STORE[(Encrypted<br/>Message Store)]
        PKS[(Pre-Key<br/>Store)]
    end
    subgraph ClientB["Client B (Recipient)"]
        B_APP[App Logic]
        B_CRYPTO[Crypto Engine]
        B_KEYS[Key Store<br/>Local / Secure Enclave]
    end
    A_APP --> A_CRYPTO
    A_CRYPTO --> A_KEYS
    ...
    B_CRYPTO --> B_KEYS
    B_CRYPTO --> B_APP
    B_CRYPTO -->|Upload pre-keys| KS`,
  components: [
    { name: 'Crypto Engine (Client-Side)', description: 'Library running on the client that handles all cryptographic operations: key generation, X3DH key agreement, Double Ratchet message encryption/decryption, and session management. Must use constant-time operations to prevent timing attacks. Implementations: libsignal (C/Rust), olm/vodozemac (Matrix).' },
    { name: 'Key Distribution Service', description: 'Server-side component that stores and distributes public pre-keys. When Alice wants to message Bob for the first time, she fetches Bob\'s pre-key bundle (identity key, signed pre-key, one-time pre-keys) from this service. The server never sees private keys.' },
    { name: 'Pre-Key Store', description: 'Database of per-device public keys uploaded by clients. Stores identity keys (long-term), signed pre-keys (medium-term, rotated monthly), and one-time pre-keys (single-use, replenished in batches). One-time pre-keys are deleted after distribution to ensure forward secrecy.' },
    { name: 'Message Relay', description: 'Routes encrypted message blobs between clients. The server sees: sender ID, recipient ID, timestamp, and an opaque encrypted payload. It cannot read the content. Stores messages for offline recipients and delivers when they connect.' },
    { name: 'Local Key Store', description: 'Secure client-side storage for private keys and session state. Uses the OS secure enclave (iOS Keychain, Android Keystore) or encrypted local storage. Keys never leave the device. Lost device = lost keys (new device requires re-establishing sessions).' },
    { name: 'Safety Number / Key Verification', description: 'Mechanism for users to verify each other\'s identity keys out-of-band (QR code scan, number comparison). Detects man-in-the-middle attacks where the server substitutes its own keys. Signal and WhatsApp show "safety numbers" for this purpose.' },
  ],
  dataModel: `erDiagram
    DEVICE {
        string device_id PK
