...
export const gmail: SystemDesign = {
  slug: 'gmail',
  name: 'Gmail',
  ...
  highLevelDiagram: `graph TB
    ...
    SMART --> ML`,
  components: [
    { name: 'SMTP / MX Service', description: 'Receives inbound emails via SMTP from other mail servers. Google operates MX (Mail Exchange) servers at multiple locations worldwide. Handles TLS encryption, DKIM/SPF/DMARC validation, and initial connection-level spam filtering (IP reputation).' },
    { name: 'Spam Filter', description: 'Multi-layer spam and phishing detection that blocks 99.9%+ of threats. Combines rule-based filters, ML classifiers (trained on user reports), URL reputation scanning, attachment sandboxing, and real-time threat intelligence. Processes every inbound and outbound email.' },
    { name: 'Delivery Service', description: 'Routes emails to the correct user\'s mailbox. Handles conversation threading (grouping replies into threads), label application, and triggers for filters/rules. Writes email content to Bigtable and metadata to Spanner.' },
    { name: 'Search Service', description: 'Full-text search across all of a user\'s emails using an inverted index. Supports operators like from:, to:, subject:, has:attachment, before:, after:, label:, is:starred. Index is updated in near real-time as new emails arrive.' },
    { name: 'Label & Category Service', description: 'Manages Gmail\'s label system (labels replace folders — an email can have multiple labels). Auto-categorization ML model classifies emails into Primary, Social, Promotions, and Updates tabs based on sender patterns and content.' },
    { name: 'Smart Features', description: 'AI-powered features including Smart Reply (suggesting short responses), Smart Compose (autocomplete while typing), nudges (reminders about emails needing a response), and important-first inbox prioritization.' },
    { name: 'Notification Service', description: 'Delivers push notifications to mobile devices and manages email notification settings. Batches notifications for less important categories and respects quiet hours.' },
  ],
  dataModel: `erDiagram
    USER {
        string user_id PK
