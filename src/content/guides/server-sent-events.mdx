...
>
Use the browser's built-in **EventSource** API to establish a persistent HTTP connection. The server responds with `Content-Type: text/event-stream` and writes events in the SSE wire format (`data:`, `event:`, `id:`, `retry:` fields). The browser automatically handles reconnection with the `Last-Event-ID` header.
</Approach>
<Approach
...
>
Client sends a request, server holds it open until data is available or a timeout occurs, then responds. Client immediately re-requests. Simulates push semantics over standard HTTP. Useful as a fallback when SSE or WebSockets are blocked by corporate proxies.
</Approach>
<Approach
...
>
If the application also needs client-to-server streaming (e.g. chat input, collaborative cursors), **WebSockets** provide full-duplex communication. However, for pure server-to-client push, SSE is simpler and sufficient.
</Approach>
</ApproachList>
...
<Mermaid chart={`graph TB
    ...
    subgraph Edge["Edge Layer"]
        LB[Load Balancer HTTP/2]
        GW1[SSE Gateway 1]
        GW2[SSE Gateway 2]
    end
    subgraph Backend["Backend Services"]
        API[API Server]
        PS[Pub/Sub Bus Redis Streams]
        WK[Event Producer Workers]
    end
    subgraph Storage
        DB[(Database)]
        CACHE[(Redis Cache)]
    ...
    WK --> PS
    API --> DB
    GW1 & GW2 --> CACHE`} caption="Server-Sent Events — High-level architecture" />

## Key Components

<ComponentList>
<ComponentCard name="SSE Gateway">
Holds open HTTP connections and streams events to clients. Subscribes to a pub/sub bus (e.g. Redis Streams) for events. Each gateway instance manages thousands of concurrent connections using async I/O (Node.js streams, Go goroutines, etc.).
</ComponentCard>
<ComponentCard name="Pub/Sub Bus">
Decouples event producers from SSE gateways. Redis Streams, NATS, or Kafka act as the fan-out layer. Each gateway subscribes to relevant channels/topics and forwards matching events to connected clients.
</ComponentCard>
<ComponentCard name="Event Producer">
Backend services or workers that generate events (e.g. order status changes, new notifications). They publish structured events to the pub/sub bus with a channel key, event type, and payload.
</ComponentCard>
<ComponentCard name="Connection Registry">
Tracks which users are connected to which gateway instance. Used for targeted delivery (send event only to user X) and connection lifecycle management (heartbeats, cleanup).
</ComponentCard>
<ComponentCard name="Load Balancer">
Distributes incoming SSE connections across gateway instances. Must support long-lived HTTP connections and avoid premature timeouts. HTTP/2 multiplexing reduces the connection count issue.
</ComponentCard>
</ComponentList>
...
<Mermaid chart={`erDiagram
    ...
    SSE_CONNECTION }o--|| CHANNEL : subscribes_to`} caption="Server-Sent Events — Entity relationship diagram" />

## SSE Wire Protocol

The SSE protocol is remarkably simple — it's just a UTF-8 text stream over HTTP with a specific format:
...
## Scaling SSE Horizontally

A single SSE gateway can hold tens of thousands of connections, but at scale you need multiple instances.
...
<Mermaid chart={`graph LR
    ...
    subgraph Gateways
        G1[Gateway 1 - 5K connections]
        G2[Gateway 2 - 5K connections]
        G3[Gateway 3 - 5K connections]
    end
    P1 & P2 --> R
    R --> G1 & G2 & G3`} caption="Scaling SSE Horizontally" />

## Event Ordering and Delivery Guarantees

SSE provides **at-most-once** delivery by default — if the client misses events during a disconnect, they're gone unless you implement replay.
...
Uses SSE for real-time updates on Actions workflow runs, deployment status, and Codespaces state changes.
</Example>

<Example system="Vercel">
Streams build logs and deployment progress to the dashboard via SSE, with automatic reconnection and log replay.
</Example>

<Example system="ChatGPT">
Streams LLM token-by-token responses via SSE, enabling the progressive text rendering experience.
</Example>

<Example system="Shopify">
Uses SSE for real-time inventory and order status updates in the merchant dashboard.
</Example>
