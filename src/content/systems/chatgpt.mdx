...
<Mermaid chart={`graph TB
    ...
    CONV --> FILEDB
    SCHED --> CACHE
    AUTH --> SAFETY`} caption="ChatGPT — High-level system architecture" />

## Key Components

<ComponentList>
<ComponentCard name="API Gateway & Rate Limiter">
Handles authentication (API keys, session tokens), rate limiting (per-user, per-tier), and request routing. Implements token-based rate limits — each request consumes tokens proportional to input + output length.
</ComponentCard>
<ComponentCard name="Model Router">
Routes requests to the appropriate model cluster based on the requested model (GPT-4, GPT-4o, GPT-4o-mini), user tier (free vs Plus vs Enterprise), and current cluster load. Implements overflow routing when primary clusters are at capacity.
</ComponentCard>
<ComponentCard name="Conversation Manager">
Manages multi-turn conversation state. Assembles the full prompt from conversation history, system prompts (for Custom GPTs), and tool results. Handles context window management — truncating or summarizing old messages when the conversation exceeds the model's context length.
</ComponentCard>
<ComponentCard name="Request Scheduler">
The core inference scheduler that batches incoming requests for efficient GPU utilization. Implements continuous batching — new requests join a running batch without waiting for all current requests to complete. Manages the KV cache allocation on GPU memory.
</ComponentCard>
<ComponentCard name="GPU Inference Cluster">
Clusters of NVIDIA H100/A100 GPUs running model inference. Each model instance is distributed across multiple GPUs using tensor parallelism (within a node) and pipeline parallelism (across nodes). Uses custom inference kernels optimized with CUDA.
</ComponentCard>
<ComponentCard name="Tool Orchestrator">
Coordinates tool calls when the model decides to use tools. Executes code interpreter sessions in sandboxed containers, fetches web pages for browsing, triggers DALL-E for image generation, and runs retrieval against uploaded documents.
</ComponentCard>
<ComponentCard name="Safety Classifier">
Runs content moderation on both inputs and outputs. A separate lightweight model classifies content against policy categories. Operates inline with low latency to avoid delaying responses.
</ComponentCard>
</ComponentList>
...
<Mermaid chart={`erDiagram
    ...
    USER ||--o{ CUSTOM_GPT : creates`} caption="ChatGPT — Entity relationship diagram" />

## LLM Inference and Continuous Batching

Serving a large language model efficiently is the central technical challenge of ChatGPT's architecture.
...
<Mermaid chart={`sequenceDiagram
    ...
    SCHED->>GPU: Batch [B, C]
    Note over GPU: Slot freed, C joins immediately`} caption="LLM Inference and Continuous Batching" />

## Tool Use and Function Calling

ChatGPT's tool use system allows the model to interact with external services — a key architectural extension beyond pure text generation.
...
## GPU Cluster and Model Parallelism

GPT-4 class models have over a trillion parameters and cannot fit on a single GPU. Serving them requires distributed inference across multiple GPUs.
