...
Instagram is one of the world's largest social media platforms, serving over 2 billion monthly active users. Its architecture handles the challenge of generating personalized feeds from billions of posts, processing massive volumes of image and video uploads, and delivering real-time interactions. Originally built as a Django monolith by a team of 3, it has evolved into a sophisticated microservices architecture within the Meta infrastructure.
...
<Mermaid chart={`graph TB
    ...
    MS --> S3 & CS
    NS --> RD
    FS --> RS`} caption="Instagram — High-level system architecture" />

## Key Components

<ComponentList>
<ComponentCard name="Feed Service">
Generates personalized feeds using a hybrid push/pull model. Pre-computes feeds for active users (fanout-on-write) and computes on-demand for inactive users (fanout-on-read).
</ComponentCard>
<ComponentCard name="Media Service">
Handles image and video upload, processing (filters, resizing, transcoding), and storage. Generates multiple resolutions for different device sizes.
</ComponentCard>
<ComponentCard name="User Service">
Manages user profiles, the social graph (follow/unfollow), and authentication. The social graph uses a TAO-based system for efficient traversal.
</ComponentCard>
<ComponentCard name="Story Service">
Manages ephemeral content with 24-hour TTL. Uses a separate storage pipeline optimized for write-heavy, short-lived data.
</ComponentCard>
<ComponentCard name="Recommendation Service">
Powers the Explore page and suggested follows using collaborative filtering and deep learning models trained on user interaction data.
</ComponentCard>
<ComponentCard name="Search Service">
Full-text search across users, hashtags, and locations using Elasticsearch with custom analyzers for multilingual support.
</ComponentCard>
</ComponentList>
...
<Mermaid chart={`erDiagram
    ...
    POST ||--o{ COMMENT : has`} caption="Instagram — Entity relationship diagram" />

## Feed Generation Architecture

Instagram uses a **hybrid fanout model** for feed generation, combining push and pull strategies:
...
<Mermaid chart={`graph TB
    ...
    end`} caption="Feed Generation Architecture" />

## Media Processing Pipeline

Every photo uploaded to Instagram goes through a sophisticated processing pipeline:
...
## Social Graph with TAO

Instagram uses Meta's **TAO (The Associations and Objects)** system for the social graph — a distributed, highly-cached graph data store.
