...
<Mermaid chart={`graph TB
    ...
    TRAFFIC --> PROBE
    GEO --> SPATIAL & POI
    SV --> IMAGERY`} caption="Google Maps — High-level system architecture" />

## Key Components

<ComponentList>
<ComponentCard name="Tile Renderer">
Generates vector map tiles at 20+ zoom levels. Tiles are pre-rendered for popular areas and generated on-demand for less trafficked regions. Uses Google's S2 geometry library for spatial partitioning. Tiles are served from edge CDN with aggressive caching.
</ComponentCard>
<ComponentCard name="Routing Engine">
Computes optimal routes using a modified Contraction Hierarchies algorithm on a preprocessed road graph. Considers real-time traffic, road restrictions, tolls, and user preferences. Supports driving, walking, cycling, and transit with different graph weights.
</ComponentCard>
<ComponentCard name="Traffic Service">
Aggregates anonymized location data from billions of Android phones and Waze users to compute real-time traffic speeds on every road segment. Updates traffic estimates every 1-2 minutes. Feeds into routing for dynamic ETA calculation.
</ComponentCard>
<ComponentCard name="Places Service">
Manages the database of 250M+ places (businesses, landmarks, addresses). Handles place search, autocomplete, details, and reviews. Uses a combination of geospatial indexing (S2 cells) and text search for queries like "coffee near me".
</ComponentCard>
<ComponentCard name="Geocoding Service">
Converts between human-readable addresses and geographic coordinates (lat/lng). Uses a combination of address parsing, interpolation along road segments, and building footprint matching.
</ComponentCard>
<ComponentCard name="Street View Service">
Serves panoramic street-level imagery captured by camera cars, trekkers, and user contributions. Imagery is stored as tiled panoramas with depth maps, enabling smooth navigation between capture points.
</ComponentCard>
</ComponentList>
...
<Mermaid chart={`erDiagram
    ...
    ROUTE_REQUEST }o--o{ ROAD_SEGMENT : uses`} caption="Google Maps — Entity relationship diagram" />

## Routing with Contraction Hierarchies

Google Maps computes billions of routes daily. Running Dijkstra's algorithm on a raw road graph with billions of edges would be far too slow. Instead, it uses **Contraction Hierarchies (CH)**.
...
<Mermaid chart={`graph TB
    ...
    Original --> |"Contraction\nPreprocessing"| Contracted`} caption="Routing with Contraction Hierarchies" />

## S2 Geometry and Spatial Indexing

Google Maps uses the **S2 Geometry Library** for all geospatial indexing and partitioning.
...
## Real-time Traffic from Crowdsourced Data

Google Maps' traffic data comes primarily from **anonymized location signals** from Android phones and Waze users.
